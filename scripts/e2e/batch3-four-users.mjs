#!/usr/bin/env node
/**
 * BATCH-3 browser check: the screens this batch touched × the six checks, three
 * admins A / B / C (sunny.b / floyd.dsil / ronlind.mene) plus a fourth browser D
 * signed in as a low-permission employee (nikhil.pati) and, where a restricted
 * editor is needed, F (ravi.kuma, function head without pay / personal flags).
 *
 *   BASE_URL=http://127.0.0.1:3016 DATABASE_URL=postgres://…/aliens_apms_b3 PORT=3016 OUTPUT=.output \
 *   A_USER=sunny.b A_PASS=0000 B_USER=floyd.dsil B_PASS=… C_USER=ronlind.mene C_PASS=… \
 *   node scripts/e2e/batch3-four-users.mjs [scenario …]
 *
 * Scenarios: scripts/e2e/lib/screens-batch3.mjs (sign-in lock-out, sessions
 * ended, default-pin switch, employee D, restricted editor F) and
 * scripts/e2e/lib/screens-targets-links.mjs (Targets / Rewards broken links,
 * second group, import blank-keep). PORT + OUTPUT let the runner restart the
 * server (APMS_DEFAULT_PIN off / on); the default-pin scenario runs last.
 */
import { writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { chromium } from "playwright";
import { makeCtx, realWrites, brief, ScreenResult } from "./lib/harness.mjs";
import * as b3 from "./lib/screens-batch3.mjs";
import * as links from "./lib/screens-targets-links.mjs";
const BASE = process.env.BASE_URL || "http://127.0.0.1:3010";
const DATABASE_URL = process.env.DATABASE_URL || "";
const OUT = process.env.OUT || "e2e-batch3.json";
const USERS = {
...
/** Run order: security screens, Targets / Rewards links, then the server-restart scenario. */
const B3 = b3.SCENARIOS.filter(([n]) => n !== "default-pin");
const PIN = b3.SCENARIOS.filter(([n]) => n === "default-pin");
export const SCENARIOS = [...B3, ...links.SCENARIOS, ...PIN];
export const SCREENS = [...b3.SCREENS, ...links.SCENARIOS.map(([n]) => ({ module: "Targets", screen: n, scenario: n, reached: true }))];
...
async function main() {
  ...
  ctx.users = USERS;
  ctx.signIn = signIn;
  if (process.env.PORT && process.env.OUTPUT) {
    ctx.restart = async (env = {}) => {
      execFileSync("sh", ["scripts/e2e/restart-server.sh"], {
        env: { ...process.env, LOG: process.env.SERVER_LOG || "/tmp/apms-b3-server.log", ...env },
        stdio: "ignore",
      });
      for (let i = 0; i < 120; i++) {
        const ok = await fetch(BASE + "/api/auth/get-session").then((r) => r.status === 200, () => false);
        if (ok) break;
        await ctx.sleep(250);
      }
      await ctx.sleep(3000); // open tabs reconnect their live stream
    };
  }
  const results = [];
