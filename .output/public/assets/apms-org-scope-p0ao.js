...
// @ts-nocheck
/** Overlay current-period roster onto identity people for Org / Rewards / APMS lists. */
function __rosterOverlay(p) {
  try {
    var c = typeof window !== "undefined" ? window.__apmsRoster : null;
    if (!c || !c.byPerson || !p || !p.id) return p;
    var a = c.byPerson[p.id];
    if (!a) return p;
    return Object.assign({}, p, {
      buId: a.sbuId == null ? "" : a.sbuId,
      managerId: a.managerId == null ? "" : a.managerId,
      functionId: a.functionId == null ? "" : a.functionId,
      brandId: a.brandId == null ? "" : a.brandId,
      companyId: a.companyId == null ? "" : a.companyId,
...
function __rosterPeople(people) {
  if (!Array.isArray(people)) return people || [];
  return people.map(__rosterOverlay);
}
...
export function accountableSbuSet(viewer, people) {
  viewer = __rosterOverlay(viewer);
  people = __rosterPeople(people);
...
export function scopedTeamIds(people, viewer, functions, units) {
  ...
  while (queue.length) {
    const bossId = queue.shift();
    for (const person of people) {
      if (!person || visited.has(person.id) || person.id === bossId) continue;
      if ((person.status || "active") === "left") continue;
      if (!reportsEdge(person, bossId)) continue;
...
export function sbuUnitsForViewer(viewer, units, people) {
  viewer = __rosterOverlay(viewer);
  people = __rosterPeople(people);
  units = units || [];
  return unitsWithPeople(units, people || []);
}
...
export function nestPeopleByPrimary(people) {
  ...
  return {
    ids,
    kids: (id) =>
      list.filter((p) => p.managerId === id || (p.dottedLine || []).some((d) => d && d.managerId === id)),
